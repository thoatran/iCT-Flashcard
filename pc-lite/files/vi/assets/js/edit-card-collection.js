...
$(document).ready(function(){
    ...
    LocalFlashcardList.remoteSync = function(cb) {
        ...
        $.ajax({
            url: SERVER_ADDRESS+"/api/flashcard/updateall",
            type: "post",
    ...
    LocalFlashcardList.createNewCard = function(currentCardId) {
        ...
        // Create new flashcard from remote
        $.ajax({
            url: SERVER_ADDRESS+"/api/flashcard/create",
            type: "post",
            data: {"username": localStorage.username,
                "collection_id": G_collection_id,
                "token": localStorage.token
    ...
    // Get collection info
    $.ajax({
        url: SERVER_ADDRESS+"/api/collection/get",
        type: "post",
        data: {"username": localStorage.username,
            "token": localStorage.token,
            "collection_id" : G_collection_id
    ...
    // Load Flashcards
    $.ajax({
        url: SERVER_ADDRESS+"/api/flashcard/getall",
        type: "post",
        data: {"username": localStorage.username,
            "collection_id": G_collection_id,
            "token": localStorage.token
    ...
    function ajaxUploadFile(base64, callback) {
        ...
        $.ajax({
            type: "POST",
            url: SERVER_ADDRESS+"/api/image/upload",
            data: data,
            dataType: "json",
            success: function (response) {
                console.log(response);
        ...
        if (file.files.length) {
            ...
            reader.onload = function (e) {
                ajaxUploadFile(reader.result, function(response) {
                    ...
                    // Update info 
                    $.ajax({
                        url: SERVER_ADDRESS+"/api/collection/update",
                        type: "post",
                        data: {"username": localStorage.username,
                            "collection_id": G_collection_id,
                            "token": localStorage.token,
        ...
        if (file.files.length) {
            ...
            reader.onload = function (e) {
                ajaxUploadFile(reader.result, function(response) {
                    ...
                    // Update info 
                    $.ajax({
                        url: SERVER_ADDRESS+"/api/flashcard/update",
                        type: "post",
                        data: {"username": localStorage.username,
                            "token": localStorage.token,
