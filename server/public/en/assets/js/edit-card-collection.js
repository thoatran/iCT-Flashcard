...
$(document).ready(function(){
    ...
    LocalFlashcardList.remoteSync = function(cb) {
        ...
        $.ajax({
            url: "/en/api/flashcard/updateall",
            type: "post",
            ...
            success: function (response) {
                ...
                if (!response.success) {
                    ...
                    alert("Success to save all flashcard collection");
                }
    ...
    LocalFlashcardList.add = function(inp, shouldPrepend, insertIndex) {
        ...
        $("#flashcard-tree").on('click', '#flashcard-' + flashcard.id + " button.delete", function(){
            let txt;
            let r = confirm("You wwant to delete the flashcard of word: \""+flashcard.word+"\" ?");
            if (r == true) {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response['success'] != false) {
                    ...
                    alert("Some errors happened when saving the collection");
                }
                cb();
            },
            error: function(jqXHR, textStatus, errorThrown) {
                alert("Some errors happened when saving collection information");
                cb();
            }
        });
    }
